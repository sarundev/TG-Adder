...
class RobustSQLiteSession(SQLiteSession):
    ...
    def _connect(self):
        self._conn = sqlite3.connect(self.filename, timeout=30, check_same_thread=False)
...
async def _promote_admin_task(req: PromoteAdminRequest):
    global LOG_BUFFER
    admin_clean = req.admin_account.replace(".session", "")
    log_msg(f"👑 Starting Admin Promotion in {req.target_chat} using {admin_clean}...")
    
    try:
        admin_client = make_client(os.path.join(ACCOUNTS_DIR, admin_clean))
        await admin_client.connect()
        if not await admin_client.is_user_authorized():
            log_msg(f"❌ Admin account {admin_clean} is unauthorized!")
            return
        ...
        for i, acc in enumerate(req.accounts_to_promote):
            acc_clean = acc.replace(".session", "")
            if acc_clean == admin_clean:
                log_msg(f"⚠️ [{i+1}/{len(req.accounts_to_promote)}] Skipping {acc_clean}: Cannot promote the admin account itself!")
                continue
                
            log_msg(f"🛡️ [{i+1}/{len(req.accounts_to_promote)}] Promoting {acc_clean}...")
            
            try:
                target_username = None
                
                # 1. Connect to the target account to ensure they have a username and have joined the group
                target_client = make_client(os.path.join(ACCOUNTS_DIR, acc_clean))
                await target_client.connect()
                
                if not await target_client.is_user_authorized():
                    log_msg(f"⚠️ Account {acc_clean} is unauthorized or missing session. Will fallback to phone contact import.")
                    ...
                    try:
                        ...
                        if not target_username:
                            import uuid
                            new_un = f"user_{uuid.uuid4().hex[:8]}_{acc_clean[-4:]}"
                            try:
                                await target_client(functions.account.UpdateUsernameRequest(new_un))
                                target_username = new_un
                                log_msg(f"   ✅ Assigned new username @{target_username}")
                            except Exception as e:
                                log_msg(f"   ⚠️ Could not set username for {acc_clean}: {e}")
                        
                        # Attempt to join the group to make promotion easier
                        try:
                            await target_client(JoinChannelRequest(target_entity))
                        except Exception:
                            pass
                    except Exception as e:
                        log_msg(f"   ⚠️ Target client error for {acc_clean}: {e}")
                    finally:
                        await target_client.disconnect()
                ...
                if target_username:
                    try:
                        user_entity = await admin_client.get_entity(target_username)
                    except Exception:
                        pass
                
                if not user_entity:
                    # Fallback to phone number import
                    contact = InputPhoneContact(client_id=0, phone=f"+{acc_clean}", first_name=f"Acc {acc_clean[-4:]}", last_name="")
                    result = await admin_client(ImportContactsRequest([contact]))
                    
                    if not result.users:
                        log_msg(f"❌ Failed to resolve {acc_clean} via username or phone number. Skipping.")
                        continue
                ...
                log_msg(f"✅ Successfully promoted {acc_clean} to Admin!")
                
            except Exception as e:
                log_msg(f"❌ Failed to promote {acc_clean}: {str(e)}")
...
def debug_db():
    """Check database connection status – use this to diagnose Render issues."""
    import sys
    db_url = os.getenv("DATABASE_URL", "not set")
    # Mask password for security
    if "@" in db_url:
        parts = db_url.split("@")
        db_url = f"postgresql://***:***@{parts[-1]}"
    return {
...
async def get_user_accounts(username: Optional[str] = None):
    """
    Return Telegram accounts associated with a web user.
    """
    if not username:
    ...
    try:
        ...
        log_msg(f"❌ Error in get_user_accounts: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")

@app.delete("/api/web/user/accounts/{phone}")
async def delete_user_account(phone: str, username: Optional[str] = None):
    """
    Delete a Telegram account linked to a web user.
    """
    if not username:
        raise HTTPException(status_code=400, detail="username query param required")
    try:
        from database import db_delete_telegram_account
        deleted = db_delete_telegram_account(phone, username)
        if deleted:
            return {"status": "success", "message": f"Account {phone} deleted"}
        raise HTTPException(status_code=404, detail="Account not found")
    except HTTPException:
        raise
    except Exception as e:
        log_msg(f"❌ Error in delete_user_account: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
try:
    from database import engine, db_load_licenses, db_save_licenses, db_load_web_users, db_save_web_users, db_add_telegram_account, db_get_user_accounts, db_delete_telegram_account
    _db_import_error = None
except Exception as _db_import_error:
    # Catches ImportError AND AssertionError from SQLAlchemy/Python 3.14 incompatibility
    print(f"[DB] ⚠️ Database import failed: {_db_import_error}")
    engine = None
    db_load_licenses = db_save_licenses = db_load_web_users = db_save_web_users = None
    db_add_telegram_account = db_get_user_accounts = db_delete_telegram_account = None

if engine:
    print("[DB] ✅ Database connected successfully")
else:
    print(f"[DB] ❌ No database connection. Error: {_db_import_error}")
...
def get_user_accounts(username: str):
    ...
    db_accounts = db_get_user_accounts(username)
    formatted = []
    from datetime import datetime, timezone
    
    for acc in db_accounts:
        # Use the exact filename stored in DB (already includes .session)
        session_fname = acc['session_filename']
        
        session_path = os.path.join(ACCOUNTS_DIR, session_fname)
        
        # Calculate expire date (e.g., 30 days from created_at, or never if not set)
        expire_str = "Never"
        if acc.get('created_at'):
            try:
                created_dt = datetime.fromisoformat(acc['created_at'])
                expire_str = (created_dt.replace(year=created_dt.year + 1)).strftime("%Y-%m-%d")
            except:
                pass
                
        formatted.append({
            "phone": acc['phone'],
            "session_filename": acc['session_filename'],
            "name": f"User {acc['phone'][-4:]}",  # Ideally fetched dynamically, but mocked for speed
            "status": "active" if is_active else "inactive",
            "expire": expire_str
        })
    return {"status": "success", "accounts": formatted}

@app.delete("/api/web/user/accounts/{phone}")
def delete_user_account(phone: str, username: str):
    if not engine:
        return {"status": "error", "detail": "Database not configured"}
    
    # 1. Get the account info first
    accounts = db_get_user_accounts(username)
    target_acc = next((a for a in accounts if a['phone'] == phone), None)
    
    if not target_acc:
        raise HTTPException(status_code=404, detail="Account not found")
        
    # 2. Delete from database
    success = db_delete_telegram_account(phone, username)
    if not success:
        raise HTTPException(status_code=500, detail="Failed to delete account from database")
        
    # 3. Delete session file
    session_fname = target_acc['session_filename']
    if not session_fname.endswith('.session'):
        session_fname += '.session'
...
async def login_request(req: LoginRequest):
    ...
    try:
        ...
        traceback.print_exc()
        await client.disconnect()
        if "phone number is invalid" in str(e).lower():
            raise HTTPException(status_code=400, detail="Invalid phone number format. Please include the country code (e.g. +1234567890)")
        raise HTTPException(status_code=500, detail=str(e))
...
async def scrape_group(req: ScrapeRequest):
    ...
    try:
        ...
        if "ChatAdminRequiredError" in error_name:
            raise HTTPException(status_code=403, detail="Cannot scrape: You must be an ADMIN to see members of this group/channel, or members are hidden.")
        raise HTTPException(status_code=400, detail=f"Scrape failed ({error_name}): {error_msg}")
    finally:
        await client.disconnect()

@app.post("/api/scraper/find-channels")
async def find_channels_api(req: FinderRequest):
    session_name = req.account
    ...
    try:
        ...
        log_msg(f"🔍 [Finder] Searching channels automatically for region: '{req.region}'...")
        ...
        for kw in keywords_to_try:
            if len(found_channels) >= req.limit:
                break
                
            try:
                result = await client(functions.contacts.SearchRequest(q=kw, limit=100))
                for chat in result.chats:
                    ...
                    if getattr(chat, 'broadcast', False):
                        ...
                        try:
                            ...
                            if count is not None and count >= req.min_members:
                                username = chat.username or ""
                                link = f"https://t.me/{username}" if username else "Private"
                                title = chat.title or ""
                                found_channels.append({
                                    'id': chat.id,
                                    'username': username,
                                    'first_name': title,
                                    'last_name': f"Members: {count}",
                                    'phone': link,
                                    'is_bot': 'No'
                                })
                        except Exception:
                ...
                log_msg(f"⚠️ [Finder] Search error for '{kw}': {e}")
                
        cache_id = str(uuid.uuid4())
        SCRAPED_MEMBERS_CACHE[cache_id] = found_channels
        
        log_msg(f"✅ [Finder] Found {len(found_channels)} channels.")
        return {"status": "success", "count": len(found_channels), "cache_id": cache_id}
        
    except Exception as e:
        log_msg(f"❌ [Finder] Error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        await client.disconnect()
...
async def check_account_can_add(session: str, target_group: str) -> tuple:
    ...
    if session.endswith('.session'):
        ...
        session_path = os.path.join(ACCOUNTS_DIR, f"{session}.session")
    client = make_client(session_path)
    try:
        await client.connect()
...
async def group_to_group_invite_worker(accounts: List[str], primary_account: str, source_group: str, target_group: str, delay: float):
    ...
    primary_path = os.path.join(ACCOUNTS_DIR, primary_account.replace('.session', ''))
    primary_client = make_client(primary_path)
    scraped_users = []
