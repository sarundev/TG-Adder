...
export default function WebTool({ onBack }) {
  ...
  const [finderRegion, setFinderRegion] = useState('cambodia');
  const [finderMinMembers, setFinderMinMembers] = useState(100000);
  const [finderLimit, setFinderLimit] = useState(100);
  const [finderAccount, setFinderAccount] = useState('');
  const [finderCacheId, setFinderCacheId] = useState(null);
  ...
  // Promoter State
  const [promoterAdminAccount, setPromoterAdminAccount] = useState('');
  const [promoterTarget, setPromoterTarget] = useState('');
  const [promoterSelectedAccounts, setPromoterSelectedAccounts] = useState([]);
  ...
  const handleDeleteAccount = async (phone) => {
    if (!window.confirm(`Are you sure you want to delete the account +${phone}?`)) return;
    try {
      const res = await fetch(`/api/web/user/accounts/${phone}?username=${formData.username}`, {
        method: 'DELETE',
      });
      const data = await res.json();
  ...
  const handlePromoter = async () => {
    if (!promoterAdminAccount || !promoterTarget || promoterSelectedAccounts.length === 0) {
      setActionMessage('Please fill all fields (owner account, target group/channel, and accounts to promote).');
      return;
    }
    setActionMessage('Starting admin promotion process...');
    try {
      const res = await fetch('/api/group/promote', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          admin_account: promoterAdminAccount,
          target_chat: promoterTarget,
          accounts_to_promote: promoterSelectedAccounts,
          delay_sec: 2
        })
      });
      const data = await res.json();
      if (res.ok) {
        ...
        setActionMessage(`❌ ${data.detail || 'Error starting promoter'}`);
      }
    } catch(e) {
      setActionMessage('Server error.');
    }
  ...
  const handleScrape = async () => {
    ...
    try {
      ...
      showMessage(`Connection error: ${e.message}`, true);
    }
  };

  const handleFinder = async () => {
    if (!finderAccount) return setActionMessage('Please select an account first!');
    try {
      setActionMessage(`Starting auto channel finder for region: ${finderRegion}...`);
      const res = await fetch('/api/scraper/find-channels', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({
          account: finderAccount.replace('.session', ''),
          region: finderRegion,
          min_members: parseInt(finderMinMembers),
      ...
      const data = await res.json();
      if (data.status === 'success') {
        setActionMessage(`Channel finder completed! Found ${data.count} channels.`);
        if (data.cache_id) {
            setFinderCacheId(data.cache_id);
        }
      } else {
        setActionMessage(`Error: ${data.detail || data.message}`);
      }
    } catch(e) {
      setActionMessage(`Connection error: ${e.message}`);
    }
  };
    ...
    <div className="web-dashboard">
      <aside className="web-sidebar">
        ...
        <nav className="sidebar-nav">
          ...
          <button className={`nav-item ${activeTab === 'channel_finder' ? 'active' : ''}`} onClick={() => setActiveTab('channel_finder')}>
          ...
          <button className={`nav-item ${activeTab === 'promoter' ? 'active' : ''}`} onClick={() => setActiveTab('promoter')}>
      ...
      <main className="web-main">
        <header className="main-header">
          <h1>
            ...
            {activeTab === 'promoter' && 'Admin Promoter'}
        ...
        <div className="main-content">
          ...
          {activeTab === 'accounts' && (
            <div className="tool-module">
              ...
              {accounts.length > 0 && (
                <div className="accounts-list" style={{ marginBottom: '2rem' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1fr auto', gap: '15px', padding: '15px', background: 'rgba(255,255,255,0.02)', borderBottom: '1px solid rgba(255,255,255,0.1)', fontWeight: 'bold', color: 'rgba(255,255,255,0.7)', fontSize: '0.9rem', borderRadius: '8px 8px 0 0' }}>
                  ...
                  {accounts.map((acc, idx) => (
                    <div key={idx} style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1fr auto', gap: '15px', padding: '15px', alignItems: 'center', background: 'rgba(255,255,255,0.05)', borderBottom: '1px solid rgba(255,255,255,0.05)', transition: 'background 0.2s', ':hover': { background: 'rgba(255,255,255,0.08)' } }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        <div style={{ width: '30px', height: '30px', borderRadius: '50%', background: 'linear-gradient(135deg, #0088cc, #005580)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: '14px' }}>
                          {(acc.name || 'U')[0].toUpperCase()}
                        </div>
                        <span>{acc.name || `User ${acc.phone.slice(-4)}`}</span>
                      </div>
                      <span style={{ fontFamily: 'monospace' }}>+{acc.phone}</span>
                      <div>
                        <span style={{ background: acc.status === 'active' ? 'rgba(46, 213, 115, 0.2)' : 'rgba(46, 213, 115, 0.2)', color: acc.status === 'active' ? '#2ed573' : '#2ed573', padding: '4px 10px', borderRadius: '12px', fontSize: '0.8rem', fontWeight: 600 }}>
                          {acc.status || 'Active'}
                        </span>
                      </div>
                      <span style={{ fontSize: '0.9rem', color: 'rgba(255,255,255,0.7)' }}>{acc.expire || 'Never'}</span>
                      <button 
                        className="tg-btn-secondary" 
                        style={{ padding: '6px 12px', fontSize: '0.8rem', background: 'rgba(255, 71, 87, 0.1)', color: '#ff4757', border: '1px solid rgba(255, 71, 87, 0.3)' }}
                  ...
                  <div style={{ background: 'rgba(255,255,255,0.02)', padding: '10px', borderRadius: '0 0 8px 8px', textAlign: 'right' }}>
                    <button className="tg-btn-secondary" style={{ padding: '6px 12px', fontSize: '0.8rem' }} onClick={fetchAccounts}>↻ Refresh Status</button>
                  </div>
                </div>
              )}
          ...
          {activeTab === 'channel_finder' && (
            <div className="tool-module">
              <h2>Channel Finder</h2>
              <div className="tg-login-card" style={{ maxWidth: '400px' }}>
                <div className="tg-login-header">
                  <h4>Find Telegram Channels</h4>
                  <p>Search and extract channels by keyword and minimum members limit.</p>
                </div>
                <div className="tg-login-body">
                  <div className="tg-input-wrap">
                    <label>Select Session</label>
                    <select className="tg-input" value={finderAccount} onChange={(e) => setFinderAccount(e.target.value)}>
                      <option value="">-- Choose Account --</option>
                      {accounts.map((a, i) => <option key={i} value={a.session_filename}>{a.phone}</option>)}
                    </select>
                  </div>
                  <div className="tg-input-wrap">
                    <label>Region</label>
                    <select className="tg-input" value={finderRegion} onChange={(e) => setFinderRegion(e.target.value)}>
                      <option value="cambodia">Cambodia Only</option>
                    </select>
                  </div>
                  <div className="tg-input-wrap">
                    <label>Minimum Members</label>
                    <input className="tg-input" type="number" placeholder="100000" value={finderMinMembers} onChange={(e) => setFinderMinMembers(e.target.value)} />
                  </div>

                  <div className="tg-input-wrap">
                    <label>Target Channels Count</label>
                    <input className="tg-input" type="number" placeholder="100" value={finderLimit} onChange={(e) => setFinderLimit(e.target.value)} />
                  </div>
                  
                  <div className="tg-actions">
                    <button className="tg-btn-primary" onClick={handleFinder}>START FINDER</button>
                  </div>

                  {finderCacheId && (
                    <div style={{ marginTop: '16px', display: 'flex', gap: '10px' }}>
                      <button className="tg-btn-secondary" style={{flex: 1, padding: '10px', background: 'rgba(255,255,255,0.1)', border: 'none', borderRadius: '8px', color: 'white', cursor: 'pointer', fontWeight: 600}} onClick={() => window.open(`/api/scraper/export/${finderCacheId}`, '_blank')}>📥 Download CSV</button>
                    </div>
                  )}
                </div>
              </div>
          ...
          {activeTab === 'promoter' && (
            <div className="tool-module">
              <h2>Admin Promoter</h2>
              <div className="tg-login-card" style={{ maxWidth: '400px' }}>
                <div className="tg-login-header">
                  <h4>Promote to Admin</h4>
                  <p>Use a group/channel owner account to promote other accounts to Admin.</p>
                </div>
                <div className="tg-login-body">
                  <div className="tg-input-wrap">
                    <label>Owner/Admin Account (Select ONE)</label>
                    <select className="tg-input" value={promoterAdminAccount} onChange={(e) => setPromoterAdminAccount(e.target.value)}>
                      <option value="">-- Select Owner Account --</option>
                      {accounts.map(a => <option key={a.session_name} value={a.session_filename}>{a.phone}</option>)}
                    </select>
                  </div>
                  
                  <div className="tg-input-wrap">
                    <label>Target Group/Channel</label>
                    <input className="tg-input" type="text" value={promoterTarget} onChange={e=>setPromoterTarget(e.target.value)} placeholder="e.g. https://t.me/mygroup" />
                  </div>
                  
                  <div className="tg-input-wrap">
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                      <label style={{ margin: 0 }}>Select Accounts to Promote ({promoterSelectedAccounts.length}/{accounts.length})</label>
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <button className="tg-btn-secondary" style={{ padding: '4px 8px', fontSize: '0.8rem', borderRadius: '4px', border: 'none', background: 'rgba(255,255,255,0.1)', color: 'white', cursor: 'pointer' }} onClick={() => setPromoterSelectedAccounts(accounts.map(a => a.session_filename))}>All</button>
                        <button className="tg-btn-secondary" style={{ padding: '4px 8px', fontSize: '0.8rem', borderRadius: '4px', border: 'none', background: 'rgba(255,255,255,0.1)', color: 'white', cursor: 'pointer' }} onClick={() => setPromoterSelectedAccounts([])}>Clear</button>
                      </div>
                    </div>
                    <div style={{ maxHeight: '150px', overflowY: 'auto', background: 'rgba(0,0,0,0.2)', padding: '10px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)' }}>
                      {accounts.map((a, i) => (
                        <label key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 0', borderBottom: '1px solid rgba(255,255,255,0.05)', fontSize: '0.9rem', cursor: 'pointer' }}>
                          <input 
                            type="checkbox" 
                            checked={promoterSelectedAccounts.includes(a.session_filename)}
                            onChange={(e) => {
                              if (e.target.checked) setPromoterSelectedAccounts([...promoterSelectedAccounts, a.session_filename]);
                              else setPromoterSelectedAccounts(promoterSelectedAccounts.filter(s => s !== a.session_filename));
                            }}
                          />
                          📱 {a.phone}
                        </label>
                  ...
                  <div className="tg-actions">
                    <button className="tg-btn-primary" onClick={handlePromoter}>START PROMOTING</button>
                  </div>
                </div>
              </div>
            </div>
